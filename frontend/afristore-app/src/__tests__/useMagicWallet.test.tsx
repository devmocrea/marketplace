...
jest.mock("@/lib/magic", () => ({
  ...
  getMagicUserMetadata: (...args: unknown[]) =>
    mockGetMagicUserMetadata(...args),
  ...
  return (
    <div>
      ...
      <button
        ...
        onClick={() => loginWithEmail("test@test.com")}
      >
        email
      </button>
      <button data-testid="login-passkey" onClick={() => loginWithPasskey()}>
        passkey
      </button>
...
describe("useMagicWallet", () => {
  ...
  it("loginWithEmail sets error state on failure", async () => {
    mockIsMagicLoggedIn.mockResolvedValueOnce(false);
    mockLoginWithMagicLink.mockRejectedValueOnce(
      new Error("email login failed"),
    );
      ...
      return (
        <div>
          ...
          <button
            data-testid="login-email"
            onClick={() =>
              loginWithEmail("test@test.com").catch(() => {
                /* swallow */
              })
            }
          >

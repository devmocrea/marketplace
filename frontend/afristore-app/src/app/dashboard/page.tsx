...
export default function DashboardPage() {
  ...
  const [selectedNFTToList, setSelectedNFTToList] = useState<OwnedToken | null>(
    null,
  );

  const activeCnt = listings.filter(
      ...
      <WalletGuard actionName="To access your artist dashboard">
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 flex flex-wrap gap-2 border-b border-white/5 pb-px overflow-x-auto no-scrollbar scroll-smooth">
            ...
            >
              <Plus
                ...
                className={clsx(
                  "transition-all duration-500 group-hover:scale-125",
                  tab === "list" &&
                    "text-mint-400 drop-shadow-[0_0_8px_rgba(38,167,110,0.5)]",
                )}
              />
              New Listing
              {tab === "list" && (
            ...
            {tab === "edit" && (
              <button className="group relative flex items-center gap-3 px-6 sm:px-8 py-5 text-sm font-bold transition-all duration-500 whitespace-nowrap text-terracotta-400">
                <Edit2
                  size={18}
                  className="text-terracotta-400 drop-shadow-[0_0_8px_rgba(235,79,27,0.5)]"
                />
                ...
                {selectedNFTToList ? (
                  <ListingForm
                    prefilledCollectionAddress={
                      selectedNFTToList.collectionAddress
                    }
                  ...
                  <OwnedNFTGallery
                    onSelect={(nft) => setSelectedNFTToList(nft)}
                  />
                )}
              </div>
            ) : tab === "auction" ? (
                ...
                {isLoading ? (
                  <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-24 animate-pulse rounded-[2rem] bg-white/[0.03] border border-white/5"
                      />
                    ))}
                  </div>
                ) : listings.length === 0 ? (
                  ...
                  <div className="grid gap-6">
                    {listings.map((l: Listing) => (
                      <div
                        key={l.listing_id}
                        className="group relative flex flex-col sm:flex-row items-center justify-between gap-6 rounded-[2.5rem] bg-white/[0.03] hover:bg-white/[0.07] hover:border-white/10 transition-all duration-500 border border-white/5 p-6 shadow-2xl"
                      >
                        <div className="flex items-center gap-6 w-full sm:w-auto">
                          ...
                          <div className="flex flex-col gap-1.5">
                            <span className="text-[10px] font-mono text-white/40 tracking-wider">
                              CID: {l.metadata_cid?.slice(0, 16) || "N/A"}…
                            </span>

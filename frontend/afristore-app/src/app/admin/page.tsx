    ...
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-4 max-w-lg text-lg text-gray-600">
          This page is reserved for marketplace administrators. Please connect
          the administrator wallet to view this panel.
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-brand-50 pb-20 pt-10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          ...
          >
            <Loader2
              className={`h-4 w-4 ${isLoadingStats || isLoadingTokens ? "animate-spin" : ""}`}
            />
            Refresh Data
          </button>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <section className="rounded-3xl bg-white p-8 shadow-sm border border-brand-100">
            ...
            <p className="mb-6 text-gray-600">
              Restrict or reinstate artist permissions. Revoked artists can
              browse and buy, but cannot create new listings or auctions.
            </p>
                  ...
                  <div className="flex items-center gap-3">
                    {searchResult.isRevoked ? (
                      <>
                        ...
                        <span className="font-semibold text-red-600 uppercase tracking-tighter text-xs">
                          Revoked / Suspended
                        </span>
                      </>
                    ) : (
                      <>
                        ...
                        <span className="font-semibold text-secondary-dark uppercase tracking-tighter text-xs">
                          Active / Verified
                        </span>
                      </>
                    )}
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-end gap-2">
                  <p className="text-3xl font-bold tracking-tight">
                    {stats?.treasuryBalances.find(
                      (b) => b.asset_type === "native",
                    )?.balance || "0.00"}
                  </p>
                  <p className="mb-1 text-sm font-medium text-white/60 uppercase">
                    XLM
                  </p>
                </div>
              </div>
              ...
              {stats?.treasuryBalances && stats.treasuryBalances.length > 1 ? (
                ...
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  ...
                  <p className="text-sm text-gray-400 italic">
                    No custom assets found in treasury
                  </p>
                </div>
              )}
            </div>
          ...
          <section className="lg:col-span-2 rounded-3xl bg-white p-8 shadow-sm border border-brand-100">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div className="rounded-2xl border-2 border-gray-100 bg-gray-50/20 p-6">
                ...
                <h4 className="font-bold text-midnight-950">
                  Native Stellar (XLM)
                </h4>
                <p className="mt-1 text-xs text-gray-500">
                  Built-in default currency
                </p>
              ...
              {whitelistedTokens.map((token) => (
                <div
                  key={token}
                  className="group relative rounded-2xl border border-gray-100 bg-white p-6 shadow-sm hover:border-brand-200 transition-all"
                >
                  <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-mint-50 text-mint-600 font-bold">
                    T
                  </div>
                  <h4
                    className="font-bold text-midnight-950 truncate"
                    title={token}
                  >
              ...
              {!isLoadingTokens && whitelistedTokens.length === 0 && (
                <div className="flex h-32 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-100 p-6 text-center text-gray-400">
                  ...
                  <p className="text-xs italic">
                    No additional SRC-20 tokens whitelisted.
                  </p>
                </div>
              )}
            </div>
...
function StatCard({
  ...
  return (
    <div className="group rounded-3xl bg-white p-6 shadow-sm transition-all hover:shadow-md border border-brand-100">
      <div
        className={`mb-4 inline-flex items-center justify-center rounded-2xl p-3 text-white ${color} shadow-lg transition-transform group-hover:scale-110`}
      >
        {icon}
      </div>
      <p className="text-sm font-medium text-gray-500">{title}</p>
      <p className="mt-1 font-display text-3xl font-bold text-midnight-950">
        {value}
      </p>
    </div>
  );
